    ...
    def _encode(self, obj, context):
        if obj < 0 and not self.signed:
            raise BitIntegerError(
                ...
                obj)
        obj2 = int_to_bin(obj,
                          width=self.width(context) if callable(self.width)
                          else self.width)
        if self.swapped:
...
class MappingAdapter(Adapter):
    ...
    :param decdefault: the default return value when the object is not found
                       in the decoding mapping. if no object is given,
                       an exception is raised.
                       if ``Pass`` is used, the unmapped object will be
                       passed as-is
    :param encdefault: the default return value when the object is not found
                       in the encoding mapping. if no object is given,
                       an exception is raised.
                       if ``Pass`` is used, the unmapped object will be passed
                       as-is
    """
...
class PaddedStringAdapter(Adapter):
    ...
    :param trimdir: the direction where trimming will take place
                    ("right" or "left"). the default is "right".
                    trimming is only meaningful for building,
                    when the given string is too long.
    """
...
class CStringAdapter(StringAdapter):
    ...
    :param encoding: the character encoding to use (e.g., "utf8"), or None to
                     return raw-bytes.
                     the terminator characters are not affected by the
                     encoding.
    """
    ...
    def _encode(self, obj, context):
        return StringAdapter._encode(self, obj, context) + \
            self.terminators[0:1]
...
class ExprAdapter(Adapter):
    ...
    :param subcon: the subcon to adapt
    :param encoder: a function that takes (obj, context) and returns an
                    encoded version of obj
    :param decoder: a function that takes (obj, context) and returns an
                    decoded version of obj
...
class PaddingAdapter(Adapter):
    ...
    :param strict: whether or not to verify, during parsing, that the given
                   padding matches the padding pattern. default is False
                   (unstrict)
    """
